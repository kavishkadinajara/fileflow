...
Five content archetypes — each exercises different SFI dimensions. A benchmark that only uses one type of document gives misleadingly narrow signal.

Source files only — .md and .html sources are the ground truth. Derived formats are regenerated, not stored, so benchmark results always reflect the current conversion pipeline.

No binary files in git — .docx and .pdf are gitignored. The generate_fixtures.py script is the reproducibility story for those formats.

Short + long documents — minimal.md is an edge case for the sentence-transformer chunking logic (fewer than 5 words per chunk should fall back gracefully). The academic paper and business report are long enough to stress-test latency.
